...
const SmartAssistOutput = ({ loading }) => {
  const searchMoviesArray = useSelector(
  ...
  const searchMovies =
    searchMoviesArray.length > 0 ? searchMoviesArray.flat() : [];
            ...
            .filter(
              (filterMovies) =>
                (filterMovies?.POSTER_URL &&
                  ...
                  filterMovies?.original_language === "kr") ||
            ...
            .map((movie) => (
              ...
              >
                <div className="upcoming-poster relative mx-4 mt-4 overflow-hidden text-white shadow-lg rounded-sm bg-blue-gray-500 bg-clip-border shadow-blue-gray-500/40">
                  <img
                    src={
                      POSTER_URL + (movie?.poster_path || movie?.backdrop_path)
                    }
                    alt={movie?.title}
                ...
                <div className="p-6 pt-1">
                  <Link to={`/movies/movie-info/${movie?.id}`}>
                    <button
                      className="w-full select-none rounded-lg bg-gray-900 hover:bg-orange-600 py-3.5 px-7 text-center align-middle font-sans text-sm font-bold uppercase text-white shadow-md shadow-gray-900/10 transition-all hover:shadow-lg hover:shadow-gray-900/20 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none flex items-center justify-center"
                      type="button"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 384 512"
                        className="w-4 mr-5 fill-current text-white"
                      >
...
export default SmartAssistOutput;
