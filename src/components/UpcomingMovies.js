...
const UpcomingMovies = () => {
  const loading = useUpcomingMovies();
  const dispatch = useDispatch();
  const allUpcomingMovies = useSelector(
  ...
  return (
    <div className="container mx-auto my-5 min-h-50">
      {loading ? (
        ...
        <>
          <h1 className="flex mx-5 font-bold text-5xl">Upcoming Movies</h1>
                ...
                <SwiperSlide key={movie.id}>
                  <div className="m-5 mx-3 relative text-xs flex w-full max-w-[23rem] flex-col rounded-lg  shadow-lg">
                    <div className="upcoming-poster relative mx-4 mt-4 overflow-hidden shadow-lg rounded-sm bg-blue-gray-500 bg-clip-border shadow-blue-gray-500/40">
                      ...
                      <button
                        className={`!absolute top-4 right-4 h-8 max-h-[32px] w-8 max-w-[32px] select-none rounded-full text-center align-middle font-sans text-xs font-medium uppercase ${favorite.some(fav => fav.id === movie?.id) ? 'text-red-500' : 'text-white'} transition-all hover:bg-red-500/10 active:bg-red-500/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none`}
                        type="button"
                        onClick={() => {
                          dispatch(addFavoriteMovies({id : movie?.id , title: movie?.title , popularity: movie?.popularity , img: movie?.poster_path}));
                        }}
                      >
